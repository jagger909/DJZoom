    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.form.is_valid():
            ...
            if self.formset.is_valid():
                ...
                return redirect(
                    reverse("goods_index", kwargs={"pk": new_good.category.pk}) + "?page=" + self.request.GET[
                        "page"] + "&sort=" + self.request.GET["sort"] + "&order=" + self.request.GET["order"])
        if self.kwargs["pk"] == None:
            self.cat = Category.objects.first()
        else:
            self.cat = Category.objects.get(pk=self.kwargs["pk"])
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.form.is_valid():
            ...
            if self.formset.is_valid():
                ...
                return redirect(
                    reverse("goods_index", kwargs={"pk": self.good.category.pk}) + "?page=" + self.request.GET[
                        "page"] + "&sort=" + self.request.GET["sort"] + "&order=" + self.request.GET["order"])
        return super(GoodUpdate, self).get(request, *args, **kwargs)
...
class GoodDelete(PageNumberView, DeleteView, SortMixin, PageNumberMixin):
    ...
    def post(self, request, *args, **kwargs):
        self.success_url = reverse("goods_index",
                                   kwargs={"pk": Good.objects.get(pk=kwargs["pk"]).category.pk}) + "?page=" + \
                           self.request.GET["page"] + "&sort=" + self.request.GET["sort"] + "&order=" + \
